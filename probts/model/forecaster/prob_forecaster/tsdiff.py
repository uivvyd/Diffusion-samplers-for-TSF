...
class TSDiffCond(Forecaster):
    ...
    @torch.no_grad()
    def dpm2_sample(self, repeated_observation, repeated_observation_mask, n_samples, timestamps, repeated_features=None):
        batch_size, length, ch = repeated_observation.shape
        seq = torch.randn_like(repeated_observation)
        ...
        def inverse_lambda(lamb):
            ...
            return int(tmp / (torch.sqrt(Delta) + beta_start) / (beta_end - beta_start))
        ...
        for i in range(len(timestamps) - 1):
            ...
            t = int(timestamps[i])
            t_next = int(timestamps[i + 1])
            r = inverse_lambda((lambdas[t_next] + lambdas[t]) / 2)
            mean_cur = means[t]
            mean_next = means[t_next]
            mean_r = means[r]
            sigma_next = sigmas[t_next]
            sigma_r = sigmas[r]
            delta = lambdas[t_next] - lambdas[t]
    ...
    def sample(self, observation, observation_mask, n_samples, features=None):
        ...
        if self.solver == 'ddpm':
            ...
            return self.dpm2_sample(repeated_observation, repeated_observation_mask, n_samples, timestamps, repeated_features)
        else:
